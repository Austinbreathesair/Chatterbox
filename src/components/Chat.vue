    ...
    <b-col cols="12">
      ...
      <footer class="sticky-footer">
        <b-form @submit="onSubmit">
          <b-input-group>
            <b-from-input id="message"
                          v-model.trim="data.message"
                          placeholder="Enter your message"></b-from-input>
            <b-button type="submit"
                      variant="primary"
                      :disabled="!data.message">Send</b-button>
          </b-input-group>
        </b-form>
      </footer>
    </b-col>
...
export default {
  ...
  created() {
    let joinData = firebase
      .database()
      .ref('chatrooms/' + this.roomid + '/chats')
      .push();
    joinData.set({
      type: 'join',
      user: this.nickname,
  ...
  methods: {
    ...
    exitChat() {
      ...
      exitData.set({
        type: 'exit',
        user: this.nickname,
        message: this.nickname + ' has exited the chatroom.',
        sendDate: Date(),
      });
